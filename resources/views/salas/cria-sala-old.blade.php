...
<div class="container">
    <div class="row">
        <div class="panel panel-default">
            ...
            <form class="form" action="/salas-old" method="POST" style="text-align: left; max-width: 500px; margin: 15px auto; padding: 0px 10px">
                <label class="style1">Nome completo: *</label><br>
                <input class="form-control" type="text" name="nome_professor" data-toggle="tooltip" data-placement="top" placeholder="Nome completo" value="{{ $sala->nome_professor }}" required autofocus  ><br>
                ...
                <select class="form-control" name="modalidade" required>
                    ...
                    @foreach($modalidades as $modalidade)
                        <option value="{{$modalidade->sigla}}" @if(!$modalidade->visivel) hidden @endif>{{$modalidade->descricao}}</option>
                    @endforeach
                </select>
                <br><br>
                
                <label class="style1">Objetivo da Disciplina: *</label><br>              
                <select class="form-control" name="objetivo_sala" required>
                    <option hidden disabled selected value> -- Selecione -- </option>
                    @foreach($objetivos as $objetivo)
                        <option value="{{$objetivo->sigla}}" @if(!$objetivo->visivel) hidden @endif>{{$objetivo->descricao}}</option>
                    @endforeach
                </select>
                <br><br>
